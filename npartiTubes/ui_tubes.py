...
from npartiTools.npartiTubes import npartiTubes as nt

reload(sys.modules['npartiTools.npartiCurves'])
reload(sys.modules['npartiTools.getnParti'])
reload(sys.modules['npartiTools.npartiTubes.npartiTubes'])
...
def createCurves(*args):
    ...
    for nParti in npartiSelection:
        ...
        drawCurvesOnly = mc.checkBox('curvesOnly', q=True, v=True)
        npartiList.append(nParti)
        particleDict = nc.npartiDict(npartiList)
        folderName = nParti
        #querie the ui for the values below here
        tubes = nt.buildTubes(particleDict, tubeDiv, tubeRad, bendDet, extruOffset, folderName, animExtrusion, randOffset, aniVis, charUp, curveCNTRL,tubeDetail,drawCurvesOnly)
        inc+=1
...
def makeTubeExtGui():
	...
	# D R A W  O N L Y  C U R V E S
	mc.frameLayout(l="Draw Only Curves", la="top", bgc=(0.1, 0.1, 0.1), cll=False, cl=False, w = 200)
	mc.columnLayout(adj=True)
	mc.checkBox('curvesOnly',label="Draw Curves", bgc=(0.21, 0.67, 0.72), v=False )
	...
	mc.button(label="Create Tubes", bgc=(0.24, 0.72, 0.46), c =createCurves , h = 40 )
	mc.setParent('..')
	mc.setParent('..')
