...
def buildTubes(particleDict, tubeDiv, tubeRad, bendDet, extruOffset, folderName, animExtrusion, randOffset, aniVis, charUp, curveCNTRL, tubeDetail, drawCurvesOnly):
	print(drawCurvesOnly)
	...
	for curveParticleId in allParticleDictionary.keys():
		...
		if len(sortedKeyFrameList) > 1:
			...
			elif drawCurvesOnly == True:
				
				mc.setAttr(curveObj + '.visibility',1)
				mc.parent(curveObj, tubeGrp)
